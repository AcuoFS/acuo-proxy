...
routerInstance.post('/allocate-selection-new', (req, res, next) => {
  ...
  Promise.all([
    ...
    console.log(JSON.stringify(_.map(selectionItems, (item) => _.set('clientAssets', _.map(item.clientAssets, (group) => group.data.length)))))
    const processedItems = _.map(selectionItems, (item) => _.set('clientAssets', _.map(item.clientAssets, (group) => group.data.length))).map(selectionItem => {
